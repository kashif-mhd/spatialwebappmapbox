...
const getVisibilityValue = (idval: any) => {
...
const MapboxPage = () => {
  ...
  // const mbdraw = useRef<MapboxDraw | undefined>(null)
  let mbdraw: MapboxDraw;
  const originalAreas = useRef({values: {} as any})
  const userSlopeClasses = useRef({values: {}})
  // const saveLegalLayertodb = useRef()
  const landCoverAnalysisFormFields = useRef({landClasses: {} as any, totalArea: 0.0})
  ...
  useEffect(() => {
            ...
            try{
              if(mapRef.current){
                if (mapRef.current.getSource("db-landcover")) {
                  const src: mapboxgl.GeoJSONSource = mapRef.current.getSource('db-landcover') as mapboxgl.GeoJSONSource;
                  src.setData(JSON.stringify(geojsondata));
                }else{
            ...
            try{
              if(mapRef.current){
                if (mapRef.current.getSource("db-slope")) {
                  const src: mapboxgl.GeoJSONSource = mapRef.current.getSource('db-slope') as mapboxgl.GeoJSONSource;
                  src.setData(JSON.stringify(geojsondata));
                }else{
            ...
            if (isSelectionMode.current) {
              ...
              if(mapRef.current){
                ...
                if (featuresLayer1.length) {
                  ...
                  selectedFeature = featuresLayer1[0];
                  selectedFeatureLayer.current = 'layer-1'
                  console.log('Selected feature from Layer 1:', selectedFeature);
                    
                } else if (featuresLayer2.length) {
                  selectedFeature = featuresLayer2[0];
                  selectedFeatureLayer.current = 'legal-boundary'
                  console.log('Selected feature from Layer 2:', selectedFeature);
                } else if (featuresIntersectionLayer.length) {
                  selectedFeature = featuresIntersectionLayer[0];
                  selectedFeatureLayer.current = 'intersection-layer'
                  console.log('Selected feature from Intersection Layer:', selectedFeature);
                  
                } else if (featuresLandcoverLayer.length) {
                  selectedFeature = featuresLandcoverLayer[0];
                  selectedFeatureLayer.current = 'db-landcover'
                  console.log('Selected feature from Landcover Layer:', selectedFeature);
                  
                } else if (featuresSlopeLayer.length) {
                  selectedFeature = featuresSlopeLayer[0];
                  ...
                  console.log('Selected feature from Layer 3:', selectedFeature);
    ...
    const highlightFeature = (feature: any) => {
      if(mapRef.current){
        const highlightSource = mapRef.current.getSource('highlighted-feature') as mapboxgl.GeoJSONSource;
        if(highlightSource){
    ...
    const copyMultilayerFeature = () => {
      if (selectedFeature != null) {
        console.log('Feature copied:', selectedFeature);
      } else {
    ...
    const pasteMultilayerFeature = () => {
      if (selectedFeature != null) {
        ...
        if (selectedFeatureLayer.current !== targetLayer) {
          pasteFeatureIntoLayer(selectedFeature, targetLayer);
        } else {
    ...
    const deleteFeature = () => {
      if (selectedFeature && selectedFeatureLayer.current) {
        deleteFeatureFromLayer(selectedFeature, selectedFeatureLayer.current);
        ...
        polys.every((poly, index) => {
          polys.every((p, i) => {
            ...
            if( i > index)  {
              ...
              const diff = turf.difference(turf.featureCollection([poly, p])) as GeoJSON.Feature;
  ...
  const intersectAllFeatures = () => {
    ...
    if(mapRef.current){
      ...
      let intersectionResults: any[] = [];
  
      layer1Features.forEach(feature1 => {
          layer2Features.forEach(feature2 => {
              console.log("Feature1: ", feature1)
              console.log("Feature2: ", feature2)
              let ftrs = turf.featureCollection([feature1, feature2]) as GeoJSON.FeatureCollection<GeoJSON.Polygon | GeoJSON.MultiPolygon>
              const intersection = turf.intersect(ftrs);
              if (intersection) {
      ...
      if (intersectionResults.length > 0) {
          const intersectionSource = mapRef.current.getSource('intersection-layer') as mapboxgl.GeoJSONSource;
